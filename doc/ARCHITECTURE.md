...
- **~~API source~~ — built (§0).** `data/intervals/` implements `load({type:'id'})` against intervals.icu. What is genuinely still open on that path:
  ...
  - **Multi-account**, and **caching the activity list** (today every visit to the view re-lists; at 1 call per list load plus 1 per activity opened, nowhere near the 5000/day, 2500/15min, ~10 req/s ceilings).
